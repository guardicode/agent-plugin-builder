...
def main():
    ...
    _build_agent_plugin(build_dir, agent_plugin_manifest)
...
def _build_agent_plugin(build_dir: Path, manifest: AgentPluginManifest):
    build_options = _get_plugin_build_config(build_dir)
...
def _get_plugin_build_config(plugin_path: Path) -> AgentPluginBuildOptions:
...
def plugin_archive_filter(file_info: tarfile.TarInfo):
    EXCLUDE_FILES = [
        "__pycache__",
        ".mypy_cache",
        ".pytest_cache",
...
def _create_source_archive(build_dir: Path) -> Path:
    ...
    with tarfile.open(str(source_archive), "w:gz") as tar:
        tar.add(source_build_dir_path, arcname=source_arcname, filter=plugin_archive_filter)
